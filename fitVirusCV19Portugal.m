function res = fitVirusCV19Portugal(getDataPortugal,varargin)
...
%   own discretion.
    ...
    % get data 
    [country,C,date0] = getDataPortugal(); 
 ...
 % plot results ===========================================================%
    ...
    if plt
        ...
        tx1 = sprintf('País: %s - Modulação Epidemiológica do Vírus SARS-CoV2 pelo modelo SIR em : %s',...
...
%   b0 -- initial guess = [K r A]' or [] if calculation fails
    ...
    for i = 1:nmax
        ...
        q = C(k2)^2 - C(k3)*C(k1);
        if q <= 0
            %      fprintf('***Aviso: Estimativa Inicial q = %g  k1 = %d k2= %d k3 = %d \n',...
            %          q, k1, k2, k3)
            continue
        end
        p = C(k1)*C(k2) - 2*C(k1)*C(k3) + C(k2)*C(k3);
        if p <= 0
            %   fprintf('***Aviso: Estimativa Inicial p = %g\n',p)
            continue
        end
        K = C(k2)*p/q;
        ...
        if r < 0
            %  fprintf('***Aviso: Estimativa Inicial r = %g\n',r)
            continue
        end
        ...
        if A <= 0
            %   fprintf('***Aviso: Estimativa Inicial = %g\n',r)
            continue
        end
...
function plotData(C,date0,country,plt)
   ...
   % tx1 = sprintf('Coronavirus epidemic in %s',country);
        tx1 = sprintf('Modulação Epidemiológica do Vírus SARS-CoV2 pelo modelo SIR em %s Data: %s',...
...
function z = flambertw(n,x)
    ...
    switch n
        ...
        otherwise
             error('**Função Flamberw: Inválida n =  %g.',n)
    end
