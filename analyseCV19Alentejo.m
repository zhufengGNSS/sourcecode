function out = analyseCV19Alentejo(getDataAlentejo,day)
...
%
    ...
    % get data
    res = fitVirusCV19Alentejo(getDataAlentejo,'plt','off');
    date0 = res.date0;
    nday = res.day;
    if nargin < 2
        day = ceil(0.5*nday);
    ...
    date0 = date0 + day - 1;
    for n = day:nday
        res = fitVirusCV19Alentejo(getDataAlentejo,'day',n,'plt','off');
        k = k + 1;
        if ~isempty(res)
            R0(k) = res.R0;
            N(k) = res.N; %????????????????????????????
   ...
   % RR = log10(R0);
    ...
    %... add title
    title({sprintf('Modulação Epidemiológica do Vírus SARS-CoV2 pelo modelo SIR na Região:  %s. Estimativas Diárias.',res.country),...
   ...
   % RR = log10(R0);
    ...
    %... add title
    title({sprintf('Modulação Epidemiológica do Vírus SARS-CoV2 pelo modelo SIR na Região:  %s. Estimativas Diárias.',res.country),...
        'Taxa Diária de R_n (n=dia) face R_O'})
